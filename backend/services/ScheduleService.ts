...
export class ScheduleService {
  ...
  static async validatePlacement(poolOrConn: any, params: ValidationParams): Promise<ValidationResult> {
    ...
    const campusMismatch = secCheck.length > 0 && (Number(roomCheck[0].campus_id || 0) !== Number(secCheck[0].campus_id || 0));
    ...
    if (targetFacultyIds.length > 0) {
      ...
      const queryDaySchedules = `
        SELECT sch.id, sch.start_time, sch.end_time, r.campus_id, r.name as room_name
        FROM schedules sch
        JOIN teaching_loads tl ON sch.teaching_load_id = tl.id
      ...
      for (const ds of daySchedules) {
        if (excludeScheduleId && Number(ds.id) === Number(excludeScheduleId)) continue;
        if (Number(ds.campus_id || 0) === Number(currentCampusId || 0)) continue; // Same campus, buffer not strictly enforced here (lunch gap handles it)
  ...
  static async autoGenerate(poolOrConn: any, term_id: number, campus_id?: number): Promise<AutoScheduleResult> {
    ...
    const [rooms] = await poolOrConn.query(`
      SELECT r.id, r.name, r.type, r.capacity, r.campus_id 
      FROM rooms r 
    ...
    // Campus Isolation: track faculty_id:day_of_week -> campus_id
    const facultyCampusMap = new Map<string, number>();
    ...
    for (const load of interleavedQueue) {
      ...
      whileLoop: while (remainingToSchedule > 0.45) { // Threshold for minimum session (approx 30 mins)
        ...
        const campusRooms = rooms.filter(r => Number(r.campus_id || 0) === Number(load.campus_id || 0));
        ...
        passLoop: for (const pass of PASSES) {
          for (const day of pass.days) {
            ...
            for (let t = pass.sMin; t <= pass.eMax - durationHours; t += 0.5) {
              ...
              let bestWellnessSlot: { s: number, e: number, room: string, campusId: number | null, score: number } | null = null;
              ...
              for (const room of roomsForLoad) {
                ...
                if (wellness.minGap >= 12.0) {
                  bestWellnessSlot = { s: sAttempt, e: eAttempt, room: room.name, campusId: Number(room.campus_id), score: wellness.minGap };
                  break;
                }
              ...
              if (bestWellnessSlot) {
                ...
                // After successful placement, lock all involved faculty to this campus for THIS DAY
                if (bestWellnessSlot && bestWellnessSlot.campusId !== null && bestWellnessSlot.campusId !== undefined) {
                  loadFacs.forEach(fId => {
                    const key = `${fId}:${day}`;
                    if (!facultyCampusMap.has(key)) {
                      facultyCampusMap.set(key, bestWellnessSlot.campusId as number);
                    }
                  });
                }
  ...
  static async validateBatchIntegrity(poolOrConn: any, schedules: any[]): Promise<{ orphanedIds: number[], issues: { id: number, reason: string }[] }> {
    ...
    // 1. Get all active rooms, faculty, and sections for validation
    ...
    const [activeSections]: [any[], any] = await poolOrConn.query('SELECT id FROM sections WHERE is_archived = FALSE');
    ...
    for (const sch of schedules) {
      ...
      // Check Section
      if (sch.section_id && !validSectionIds.has(sch.section_id) && sch.section_id !== 1) { // 1 is usually the "General" section
        issuesForBlock.push(`Section ID ${sch.section_id} is archived or missing.`);
      }
